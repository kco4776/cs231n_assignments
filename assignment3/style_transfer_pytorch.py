...
def content_loss(content_weight, content_current, content_original):
    ...
    content_loss = torch.sum(torch.square(content_current.view(1, c, -1)-content_original.view(1, c, -1)))*content_weight
...
def gram_matrix(features, normalize=True):
    ...
    f = features.view(N, C, -1) # (N, C, H*W)
    tp_f = torch.transpose(f, 1, 2)
    gram = torch.matmul(f, tp_f)
    if normalize:
      gram /= C*H*W
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      style_loss += torch.sum(torch.square(gram_matrix(feats[style_layers[i]])-style_targets[i]))*style_weights[i]
...
def tv_loss(img, tv_weight):
    ...
    h = torch.sum(torch.square(img[:, :, torch.arange(1, H), :]-img[:, :, torch.arange(H-1), :]))
    v = torch.sum(torch.square(img[:, :, :, torch.arange(1, W)]-img[:, :, :, torch.arange(W-1)]))
    loss = tv_weight * (h+v)
