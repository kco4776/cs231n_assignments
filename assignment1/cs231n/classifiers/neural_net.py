...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        z = np.dot(X, W1) + b1
        relu = np.maximum(z, 0)
        scores = np.dot(relu, W2) + b2 # scores: shape(N, C)
        ...
        grads['W2'] = np.dot(relu.T, softmax) / N
        grads['b1'] = np.sum(np.dot(softmax, W2.T) * (z>0), axis=0) / N
        grads['W1'] = np.dot(X.T, np.dot(softmax, W2.T) * (z>0)) / N
